...
const Analytics = () => {
  ...
  const handleCheckBalance = async (data) => {
    try {
      const newBalance = await CPR.balanceOf(data.account);
...
const Analytics = () => {
  ...
  const [CPR, faucet] = useContext(CopyrightContext);
  const [address, setAddress] = useState(0);
  const [form, setForm] = useState({ owner: "", sender: "" });

  const handleCheckBalance = async () => {
    try {
      const balance = await CPR.balanceOf(address);
  ...
  const handleCheckAllowance = async (e) => {
    try {
      const allowance = await CPR.allowance(form.owner, form.sender);
